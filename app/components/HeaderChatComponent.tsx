...
export default function HeaderChatComponent() {
  ...
  const renderAvatar = (avatarUrl: string | null, name: string, online: boolean) => (
    <div className="relative flex-shrink-0">
      {avatarUrl ? (
        <Image src={avatarUrl} alt={name} width={44} height={44} className="rounded-full object-cover" />
      ) : (
      ...
      <div className="flex border-b border-border">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-colors ${
            activeTab === "chats" ? "text-primary border-b-2 border-primary" : "text-text-secondary hover:text-text-primary"
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-colors ${
            activeTab === "people" ? "text-primary border-b-2 border-primary" : "text-text-secondary hover:text-text-primary"
          }`}
        >
            ...
            {convLoading ? (
              ...
              <ul className="space-y-0.5 px-2">
                {conversations.map((conv) => {
                  ...
                  return (
                    <li key={conv.id}>
                      <Link href={`/chat/${conv.id}`} className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-primary/5 text-text-primary transition-all">
                        {renderAvatar(conv.other_user.avatar_url, conv.other_user.full_name, online)}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-semibold truncate">{conv.other_user.full_name}</p>
                            {conv.last_message && (
                              <span className="text-[10px] text-text-secondary flex-shrink-0">{formatTime(conv.last_message.created_at)}</span>
                            )}
                          </div>
            ...
            {usersLoading ? (
              ...
              <ul className="space-y-0.5 px-2">
                {users.map((user) => {
                  ...
                  return (
                    <li key={user.id}>
                      <button onClick={() => startChat(user.id)} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-primary/5 text-text-primary transition-all text-left">
                        {renderAvatar(user.avatar_url, user.full_name, online)}
                        <div className="flex-1 min-w-0">
